        ...
        <div class="p-4">
            @foreach($typeList as $vo)
                @if($vo['data'])
                    <div class="mb-4">
                        <div class="p-4">
                            <span class="text-base">{{$vo['name']}} </span><small
                                class="ml-2 text-gray-500">{{$vo['desc']}}</small>
                        </div>
                        <div class="px-4">
                            <div
                                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xxl:grid-cols-5 gap-4 ">
                                @foreach($vo['data'] as $data)
                                    <div>
                                        <a href="{{$data['url']}}" target="{{$data['target'] ?: '_self'}}"
                                           class="flex items-center gap-4 shadow bg-white p-4 rounded  transition hover:shadow-md ">
                                            <div class="flex-none w-10 h-10 rounded text-white p-2"
                    ...
                    <div class="px-4">
                        <div
                            ...
                            @foreach($formList as $vo)
                                <div>
                                    <a href="{{route('admin.tools.formData', ['form' => $vo->form_id])}}"
                                       class="flex items-center gap-4 shadow bg-white p-4 rounded  transition hover:shadow-md">

...
use DevEngine\Core\Exceptions\ErrorException;
...
trait Upload
{
    ...
    public function ajax(Request $request)
    {
        ...
        $dirId = \DevEngine\Core\Model\FileDir::where('dir_id', $id)->value('dir_id');
        if (empty($dirId)) {
            $dirId = \DevEngine\Core\Model\FileDir::where('has_type', $this->getHasType())->orderBy('dir_id',
                'desc')->value('dir_id');
        }
        if (empty($dirId)) {
            $dirId = \DevEngine\Core\Model\FileDir::insertGetId([
                'name' => '默认',
                'has_type' => $this->getHasType()
            ]);
        }

        $data = \DevEngine\Core\Util\Upload::load($this->getHasType(), [
    ...
    public function remote()
    {
        ...
        foreach ($data as $key => $vo) {
            ...
            \DevEngine\Core\Model\File::insert($upload);
        }

...
use DevEngine\Core\Events\ManageTable;
use DevEngine\Core\Events\ManageForm;
use DevEngine\Core\Events\ManageStatus;
use DevEngine\Core\Events\ManageClear;
use DevEngine\Core\Events\ManageRecovery;
use DevEngine\Core\Events\ManageExport;
use DevEngine\Core\Events\ManageDel;
use DevEngine\Core\UI\Event;
 ...
 * 管理端口扩展功能接口
 * Trait Expend
 * @package DevEngine\Core\Controller
 * @method \DevEngine\Core\UI\Table table()
 * @method \DevEngine\Core\UI\Form form($id = 0)
...
trait Expend
{
    ...
    protected function tableReload()
    {
        ...
        return (new \DevEngine\Core\UI\Script())->add("dux.event.event.emit('table-action-{$eventName}', 'reload')")->render();
    }
    ...
    {
        $eventName = md5($this->eventName());
        return (new \DevEngine\Core\UI\Script())->add("dux.event.event.emit('table-action-{$eventName}', 'to-page',{$page})")->render();
    }

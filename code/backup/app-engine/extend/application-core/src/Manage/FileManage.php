...
trait FileManage
{
    ...
    private function getFile($dirId, $query = '', $filter = 'all'): array
    {
        ...
        if ($dirId) {
            $data = \DevEngine\Core\Model\File::where('has_type', $this->getHasType())->where('dir_id', $dirId);
            if ($query) {
    ...
    {
        return \DevEngine\Core\Model\FileDir::where('has_type', $this->getHasType())->get()->toArray();
    }
    ...
    private function createFolder($name): array
    {
        ...
        $file = new \DevEngine\Core\Model\FileDir;
        $file->name = $name;
    ...
    private function deleteFolder(int $id): array
    {
        ...
        $files = \DevEngine\Core\Model\File::where('has_type', $this->getHasType())->where('dir_id', $id)->get([
            'driver', 'path'
        ]);
        $files->map(function ($vo) {
            Storage::disk($vo->driver)->delete($vo->path);
        });
        \DevEngine\Core\Model\File::where('file_id', $id)->delete();
        \DevEngine\Core\Model\FileDir::where('dir_id', $id)->delete();
        return [];
    }
    ...
    private function deleteFile($ids): array
    {
        ...
        $files = \DevEngine\Core\Model\File::where('has_type', $this->getHasType())->whereIn('dir_id', $ids)->get([
            'driver', 'path'
        ]);
        $files->map(function ($vo) {
            Storage::disk($vo->driver)->delete($vo->path);
        });
        \DevEngine\Core\Model\File::whereIn('file_id', $ids)->delete();
        return [];
    }

...
class Development extends Common
{
    ...
    public function index()
    {
        ...
        // 访问量
        $apiNumData = app(\DevEngine\Core\Model\VisitorApi::class)
        ...
        $apiNumChart = (new \DevEngine\Core\Util\Charts)
            ->area()
        ...
        // 访问延迟
        $apiTimeData = app(\DevEngine\Core\Model\VisitorApi::class)
        ...
        $apiTimeChart = (new \DevEngine\Core\Util\Charts)
            ->line()
            ->date(date('Y-m-d', $startTime), date('Y-m-d'), '1 days', 'm-d')
        ...
        // 文件上传
        $fileNumData = app(\DevEngine\Core\Model\File::class)
        ...
        $fileNumChart = (new \DevEngine\Core\Util\Charts)
            ->column()
            ->date(date('Y-m-d', $startTime), date('Y-m-d'), '1 days', 'm-d')
        ...
        // 操作日志
        $operateData = app(\DevEngine\Core\Model\VisitorOperate::class)
            ->select(DB::raw('COUNT(*) as value, DATE_FORMAT(created_at,"%Y-%m-%d")  as label'))
            ->where('created_at', '>=', date('Y-m-d', $startTime))
            ->where('has_type', 'admin')
            ->groupBy(DB::raw('DATE_FORMAT(created_at,"%Y-%m-%d")'))
        ...
        $logNumChart = (new \DevEngine\Core\Util\Charts)
            ->column()
            ->date(date('Y-m-d', $startTime), date('Y-m-d'), '1 days', 'm-d')

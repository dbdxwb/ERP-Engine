...
class User extends Expend
{
    ...
    protected function table()
    {
        ...
        $table = new \DevEngine\Core\UI\Table(new $parser['model']());
        $table->title('用户管理');
        ...
        $table->filter('角色', 'role_id', function ($query, $value) {
            ...
            return \DevEngine\Core\Model\Role::where('guard', 'admin')->pluck('name', 'role_id')->toArray();
        })->quick();
    ...
    {
        $parser = $this->parserData();
        $form = new \DevEngine\Core\UI\Form(new $parser['model']());
        $form->dialog(true);
        $form->setKey('user_id', $id);

        $form->select('角色', 'role_ids', function () use ($parser) {
            return \DevEngine\Core\Model\Role::where('guard', $parser['layer'])->pluck('name', 'role_id');
        }, 'roles')->multi()->verify([
            'required',
        ], [
            'required' => '请选择角色',

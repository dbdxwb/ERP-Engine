...
namespace DevEngine\Core\Admin\Tools;

use DevEngine\Core\Admin\System\Expend;
use Illuminate\Validation\Rule;

class FormData extends Expend
{

    public string $model = \DevEngine\Core\Model\FormData::class;
    protected $formInfo;

    public function __construct()
    {
        $formId = request()->get('form');
        $this->formInfo = \DevEngine\Core\Model\Form::find($formId);
    }

    protected function table()
    {
        $table = new \DevEngine\Core\UI\Table(new $this->model());
    ...
    {
        $form = new \DevEngine\Core\UI\Form();
        $form->dialog(true);
        $form->action(route('admin.tools.formData.save', ['id' => $id, 'form' => $this->formInfo->form_id]));
        app(\DevEngine\Core\Util\Form::class)->getFormUI($this->formInfo->form_id, $form, $id);
        return $form;
    }
    ...
    public function save($id = 0)
    {
        ...
        app(\DevEngine\Core\Util\Form::class)->saveForm($this->formInfo->form_id, $data, $id);

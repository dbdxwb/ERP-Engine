...
namespace DevEngine\Core\Middleware;
...
class VisitorAfter
{
    ...
    private function operate(Request $request)
    {
        ...
        // Route::get('notification', ['uses' => 'DevEngine\Core\Admin\Index@getNotify', 'ignore_operate_log' => true])->name('admin.notification');
        if ($request->route()->getAction('ignore_operate_log')) {
            return false;
        }
        $model = new \DevEngine\Core\Model\VisitorOperate();
        // 清理过期日志
    ...
    private function api($request): void
    {
        ...
        $model = new \DevEngine\Core\Model\VisitorApi;
        ...
        try {
            ...
            if (!Redis::get($key)) {
                ...
                \DevEngine\Core\Model\VisitorApi::where('api_id', $id)->increment('uv', 1);
            }
            DB::commit();
        } catch (\Exception $e) {
            DB::rollback();

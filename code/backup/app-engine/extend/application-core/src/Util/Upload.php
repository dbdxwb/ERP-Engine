    ...
    public static function load(string $hasType, array $config = [], int $dirId = 0, string $driver = ''): array
    {
        ...
        if (is_array($files)) {
            ...
            foreach ($filesData as $item) {
                ...
                $path = $file->store('public/upload/' . date('Y-m-d'), $driver);
                if ($path) {
                    $tmp = [
                        'dir_id' => $dirId,
                        'has_type' => $hasType,
                    ...
                    $ids[] = \DevEngine\Core\Model\File::insertGetId($tmp);
                } else {
                    app_error('上传失败');
                }
            }
        }
        $data = \DevEngine\Core\Model\File::where('has_type', $hasType)->whereIn('file_id', $ids)->get([

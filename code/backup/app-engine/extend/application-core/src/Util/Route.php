...
class Route
{
    ...
    {
        $classData = explode('\\', str_replace('/', '\\', $class), 5);
        [$module, $module2, $layout, $app, $as] = $classData;
        if (!$app || !$layout || !$as) {
    ...
    public function make(): void
    {
        ...
        $collection->map(function ($item) {
            ...
            foreach ($item as $vo) {
                \Route::match(is_array($vo['type']) ? $vo['type'] : [$vo['type']], $vo['rule'], ['uses'      => $vo['uses'],
                                                                                                 'desc'      => $vo['desc'],
                                                                                                 'auth_list' => isset($vo['auth_list']) ? $vo['auth_list'] : []
                ])->name($vo['name']);
            }
        });
    }

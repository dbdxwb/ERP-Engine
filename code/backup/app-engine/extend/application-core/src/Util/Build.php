...
class Build
{
    ...
    public function getBuild()
    {
        ...
        if (!is_file($buildPath = app()->bootstrapPath('cache/dev-engine.php'))) {
            $this->build();
        }
    ...
    {
        $buildPath = app()->bootstrapPath('cache/dev-engine.php');
        ...
        $list = collect($packages)->mapWithKeys(function ($package) {
            return [$this->format($package['name']) => $package['extra']['dev-engine'] ?? []];
        })->filter()->all();

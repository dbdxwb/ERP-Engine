...
namespace DevEngine\Core\UI\Form;

/**
 * 地区选择器
 * @package DevEngine\Core\UI\Form
 */
class Area extends Element implements Component
{
    ...
    {
        $info = \DevEngine\Core\Model\Area::where(['code' => $data])->first();

        $code = $info->parent_code;
        if ($info->level > 3) {
            $region = \DevEngine\Core\Model\Area::where(['code' => $code])->first();
            $code = $region['parent_code'];
        }
        if ($info->level > 2) {
            $city = \DevEngine\Core\Model\Area::where(['code' => $code])->first();
            $code = $city['parent_code'];
        }
        if ($info->level > 1) {
            $province = \DevEngine\Core\Model\Area::where(['code' => $code])->first();
        }
        $data = [];
        if ($region) {
